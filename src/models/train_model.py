...
def parse_args():
    ...
    # ✅ Made both path arguments optional to support both environments
    parser.add_argument('--models-dir', type=str, help="Directory to save models (for local runs)")
    parser.add_argument('--output-model-path', type=str, help="Full file path to save the model (for Kubeflow runs)")
...
def main(args):
    ...
    # ✅ Determine the correct save path based on provided arguments
    if args.output_model_path:
        # KUBEFLOW PATH: The full file path is provided directly
        save_path = args.output_model_path
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
    elif args.models_dir:
        # LOCAL PATH: Construct the path from the directory
        output_dir = os.path.join(args.models_dir, 'trained')
        os.makedirs(output_dir, exist_ok=True)
        save_path = os.path.join(output_dir, f"{model_name}.pkl")
    else:
        raise ValueError("Either --models-dir or --output-model-path must be provided.")
    ...
    with mlflow.start_run(run_name="final_training"):
        ...
        # ✅ Corrected description and tags to use the accurate save_path
        description = (
        ...
        client.set_registered_model_tag(model_name, "model_path", save_path)
        ...
        # ✅ Unified model saving works for both local and Kubeflow
